...
function createApp(customDb) {
    ...
    // Database setup
    const db = customDb || new sqlite3.Database('library.db', (err) => {
        if (err) {
                ...
                db.run(`CREATE TABLE IF NOT EXISTS books (
                    ...
                    db.run(`CREATE TABLE IF NOT EXISTS members (
                        ...
                        db.run(`CREATE TABLE IF NOT EXISTS borrowings (
                            ...
                            if (err) return reject(err);
                            insertSampleData(db).then(resolve).catch(reject);
                        });
                    });
                });
            });
    ...
    // Insert sample data (returns a Promise)
        ...
        return new Promise((resolve, reject) => {
            ...
            db.get("SELECT COUNT(*) as count FROM books", (err, row) => {
                ...
                if (row.count === 0) {
                    ...
                    let pending = sampleBooks.length;
                    sampleBooks.forEach(book => stmt.run(book, (err) => {
                        if (err) return reject(err);
                        if (--pending === 0) stmt.finalize((err) => {
                            if (err) return reject(err);
                            insertMembers();
                        });
                    }));
            ...
            function insertMembers() {
                db.get("SELECT COUNT(*) as count FROM members", (err, row) => {
                    ...
                    if (row.count === 0) {
                        ...
                        let pending = sampleMembers.length;
                        sampleMembers.forEach(member => stmt.run(member, (err) => {
                            if (err) return reject(err);
                            if (--pending === 0) stmt.finalize((err) => {
                                if (err) return reject(err);
                                resolve();
    ...
    // Initialize DB and expose a ready promise
    app.ready = initializeDatabase(db);

    return app;
}

// Default export for production and legacy tests
